...
export function sessionMode(session) {
  ...
  if (userMsg !== undefined) {
    const text = extractText(userMsg.data).trim()
    if (text) return classifyTask(text)
  }
  // First turn: the message is only durable as an inbox splice, not a
  // user/message event yet (the loop assembles the prompt before appending the
  // claimed messages). Read the first user-sourced inbox insertion directly.
  for (const event of events) {
    if (event.type !== 'agent/inbox/spliced') continue
    const inserted = Array.isArray(event.data?.inserted) ? event.data.inserted : []
    for (const message of inserted) {
      if (message !== null && typeof message === 'object' && message.source?.kind === 'user') {
