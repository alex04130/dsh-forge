...
dsh-forge 是运行在 `~/.dsh` 用户层的一整套 DSH 扩展，不 monkey-patch 任何 npm 包。当前规模：host 插件 15 个 / 动态插件 12 条（其中 gitdk、stfx 默认关）/ 模型工具 49 个 / npm **latest `@dsh-forge/bundle` 0.1.5** · **preview 0.2.0-preview.1（`--tag preview`，不覆盖 latest）**。
